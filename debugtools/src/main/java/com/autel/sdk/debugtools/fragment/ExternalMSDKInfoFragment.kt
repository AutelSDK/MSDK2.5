...
import com.autel.drone.sdk.LogCfg
...
class ExternalMSDKInfoFragment : AutelFragment() {
    ...
    var count = 0
    private fun addOpenDebugEvent(){
        binging.mainTitleLayout.findViewById<TextView>(R.id.title_text_view).setOnClickListener {
            SDKLog.d(TAG, "addOpenDebugEvent=$count")
            count += 1
            if(count == 5) {
                LogCfg.openDebug()
                ToastUtils.showToast("open debug log")
            }
        }
    }
}